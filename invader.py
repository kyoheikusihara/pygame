...
SCR_RECT = Rect(0, 0, 640, 480) #スクリーンサイズ
...
class Invader:
    ...
    def update(self):
        ...
        if self.game_state == PLAY:
            ...
            if self.score >= 10 and self.score < 20:
                ...
                self.alien_prob = 10
            # エイリアンを作成
            if not random.randrange(self.alien_prob):
                Alien((0, 0))
            self.all.update()
            # ミサイルとエイリアンの衝突判定
    ...
    def draw(self, screen):
        ...
        if self.game_state == START:  # スタート画面
            ...
            credit = credit_font.render("2017 pygame", False, (255,255,255))
            ...
            push_font = pygame.font.SysFont(None, 40)
            push_space = push_font.render("PUSH SPACE KEY", False, (255,255,255))
            screen.blit(push_space, ((SCR_RECT.width-push_space.get_width())/2, 250))
    ...
    def collision_detection(self):
        ...
        # プレイヤーとエイリアンの衝突判定
        player_collided = pygame.sprite.groupcollide(self.aliens, self.pc, True, True)
        for _ in player_collided.keys():  # プレイヤーと衝突したエイリアンがいれば
            self.game_state = GAMEOVER  # ゲームオーバー！
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.rect.left = 320 # プレイヤーを画面中央へ
        self.reload_timer = 0
    def update(self):
        # 押されているキーをチェック
...
class Alien(pygame.sprite.Sprite):
    """エイリアン"""
    speed = 1  # 移動速度
    animcycle = 15  # アニメーション速度
    frame = 0
    move_width = 640  # 横方向の移動範囲
    ...
    def update(self):
        # エイリアンの動き
        mov_vec = [(2 * -self.speed, 0), (0, 3 * self.speed), (2 * self.speed, 0), (0, -self.speed)]  # 左, 上, 右, 下の順で指定。
